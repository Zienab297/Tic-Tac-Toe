...
def check_draw(board):
    for row in board:
        for cell in row:
            if cell == " ":
                return False  # If an empty cell is found, the game is not a draw.
    return True

def minimax(board, depth, is_maximizing): # the computer is aiming to maximimize the score
    if check_win(board, "O"):
        return 10 #if computer wins
    if check_win(board, "X"):
        return -10 #if computer loses
    if check_draw(board):
        return 0 #if computer tie
...
def get_computer_move(board):
    best_score = -float("inf")      #min value
    best_move = (-1, -1)            #min best move
    for i in range(3):
        for j in range(3):
            if board[i][j] == " ":
                board[i][j] = "O"
